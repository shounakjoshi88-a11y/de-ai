...
- markllm — research harness (KGW / SynthID schemes) via
  deai/markllm_harness.py, activated by MARKLLM_DIR. Same-config-only
...
Vendor note (Aug 2026): Google's SynthID Text watermark remains deployed on
Gemini (Gemini API output has been watermarked since 2024; the method itself
is open source in Hugging Face Transformers >= 4.46). Third-party detection
is key-gated, and the Gemini API's own detect endpoint is not usable as a
public oracle, so no gemini-synthid-text detector ships here; the seam is
kept for a vendor endpoint that returns. Anthropic announced (Aug 14, 2026)
a SynthID-Text-style watermark on Claude models launched on/after Aug 2,
2026, and has stated a text-detection API is coming that third parties can
use; until a public endpoint exists the claude-text detector reports
unavailable. Statistical probing (deai.watermark_probe, stylometry) remains
the practical detection route for now.
"""
...
class MarkLLMTextDetector:
    ...
    def detect(self, text: str) -> dict[str, Any]:
        ...
        script = Path(__file__).resolve().parent / "markllm_harness.py"
...
class ClaudeTextDetector:
    ...
    On Aug 14, 2026 Anthropic announced an invisible SynthID-Text-style
    watermark (a Google DeepMind variant) on Claude models launched on/after
    Aug 2, 2026, shipped globally, driven by EU AI Act Article 50(2)
    transparency obligations. Anthropic stated a text-detection API that
    third parties can use is coming. No public endpoint exists yet. When it
    ships, set WATERMARKS_CLAUDE_API_KEY, flip available() to check it, and
    fill in detect() against the documented endpoint.
    """
    ...
    def detect(self, text: str) -> dict[str, Any]:
        return {
            ...
            "available": False,
            "error": (
                "Anthropic announced (Aug 14, 2026) a text-watermark detection "
                "API for Claude; it is not public yet. When it ships, set "
                "WATERMARKS_CLAUDE_API_KEY and implement ClaudeTextDetector."
            ),
        }


# ---------------------------------------------------------------------------
# Probe (de-ai built-in) — KGW-MinHash statistical detector, offline
# ---------------------------------------------------------------------------


class ProbeTextDetector:
    """Built-in KGW-MinHash watermark probe as a TextDetector.

    The same detector Layer B uses as its default evaluator, exposed through
    the detector protocol so /api/detect-watermark and detector_status()
    report it. Verdict is z vs. threshold; text below the probe's 51-token
    floor reports clean (nothing can be measured below it).
    """
    ...
    def available(self) -> bool:
        from .watermark_probe import _get_tokenizer

        return _get_tokenizer() is not None

    def detect(self, text: str) -> dict[str, Any]:
        from .watermark_probe import probe

        report: dict[str, Any] = {"detector": self.name, "vendor": self.vendor}
        try:
            result = probe(text)
        except Exception as e:  # fail-soft: a broken probe never blocks cleaning
        ...
        if result is None:
            if not self.available():
                report["available"] = False
                report["error"] = "probe tokenizer not loaded (tiktoken / tokenizers / transformers)"
                return report
            report.update(
                {
                    "available": True,
                    ...
                    "n_tokens": None,
                    "threshold": None,
                    "note": "text below the probe's 51-token operating floor",
                }
            )
            return report
        report.update(
            {
                ...
                "is_watermarked": result["verdict"] == "watermarked",
                "score": result["z"],
                "z": result["z"],
                "z_per_key": result["z_per_key"],
...
def all_detectors(
    ...
    detectors: list[TextDetector] = [ProbeTextDetector()]
    if include_markllm:
