"""Shared helpers, ported from watermarks-remover (MIT) for de-ai.

Kept for the MarkLLM harness CLI and the container pipeline: byte sniffs,
safe atomic writes, subprocess rlimits, JSON emission, and the confidence
classifier used by scanner findings.
"""
...
# via preexec_fn so a crafted file cannot make exiftool/c2patool exhaust
# host memory or fill the disk.
...
# Advice for the text-only tools: de-ai handles containers via the
# container pipeline; pass the raw text through the API instead.
TEXT_TOOL_ADVICE = (
    "Use the de-ai container endpoints for this file, which route by format,",
    "or pass allow_binary=True to scan the raw bytes anyway.",
)

# Advice for the container routers. The bytes match no supported text,
# image or container format, so pointing back at them would be circular.
ROUTER_ADVICE = (
    "These bytes match no supported text, image or container format.",
    "Pass allow_binary=True to handle them as text anyway.",
)
