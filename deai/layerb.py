"""Layer B rewrite hook for statistical (token-sampling) watermarks.

Ported from watermarks-remover (MIT) and extended with a deterministic
generator that needs no model.

Backends:
  deterministic      — de-ai's own deterministic rewrite engine (default; no
                       model, no network). Candidates are generated at
                       increasing deep-rewrite scrub depth so each variant
                       is a genuinely different rewrite pass.
  print-prompt       — emit prompt only (CI-safe, no model)
  ollama             — POST to Ollama /api/chat
  openai-compatible  — POST to OpenAI-style /v1/chat/completions

Env (optional, LLM backends only):
...
same-config detection (when --markllm-scheme is passed) > the built-in
KGW-MinHash probe (statistical green-list bias; tiktoken GPT-2 BPE, offline)
> bigram-Jaccard lexical divergence (no pass/fail verdict — all attempts are
generated and the most diverged one is selected). A vendor-detector seam
(Claude's announced detection API; Google's SynthID-text remains deployed on
Gemini but has no public third-party detector) is reserved ahead of MarkLLM
should a vendor endpoint return.

Security notes (LLM backends):
...
from .text_detectors import MarkLLMTextDetector, ProbeTextDetector
...
# Deterministic backend: candidate i (across loops) is a deep_rewrite at
# scrub depth tiers[i]. Tier 0 is a single paraphrase pass; higher tiers
# re-run the whole deep-rewrite pipeline on the previous result. Tiers are
# spread so consecutive candidates differ in depth, and loops keep producing
# fresh variants (deterministic backends would otherwise repeat loop 1).
_DETERMINISTIC_SCRUB_TIERS = (0, 1, 2, 3, 5, 8)
...
def _pick_evaluator(
    ...
    --markllm-scheme) > the built-in KGW-MinHash probe (a real pass/fail
    verdict, no external deps beyond tiktoken) > bigram-Jaccard lexical
    divergence (fallback with no pass/fail verdict). A vendor-detector seam
    (Claude's announced watermark-detection API; Google's SynthID-text has no
    public third-party detector) is reserved ahead of MarkLLM should a vendor
    endpoint return; it only needs available()/detect()/name per the
    TextDetector protocol in text_detectors.py.
    """
    if markllm_detector is not None:
        return "markllm", markllm_detector
    probe_eval = ProbeTextDetector()
    if probe_eval.available():
        return "probe", probe_eval
    return "lexical-divergence", None


def _generate_deterministic(text: str, tier: int, profile: dict | None) -> str:
    """Generate one variant with de-ai's deterministic engine (no model).

    The variant is a deep_rewrite at scrub depth *tier*: 0 is a single
    paraphrase pass, and higher tiers re-run the rewrite pipeline on the
    previous output. Deterministic given the same input.
    """
    from .deeprewrite import deep_rewrite

    result = deep_rewrite(text, max_scrub=tier, profile=profile)
    return result["text"]
...
def _generate_once(
    ...
    *,
    ...
    if backend == "deterministic":
        return _generate_deterministic(original_text, tier, profile)
    if backend == "ollama":
    ...
    *,
    ...
    if markllm_scheme:
        ...
        if not markllm["before"]["available"]:
            print(f"markllm verification unavailable: {markllm['before']['error']}", file=sys.stderr)
        info["markllm"] = markllm

    if backend == "print-prompt":
        info["mode"] = "print-prompt"
        if candidates > 1:
            print("note: --candidates ignored in print-prompt mode", file=sys.stderr)
        return prompt, info

    if backend not in ("deterministic", "ollama", "openai-compatible"):
        raise SystemExit(f"unknown backend: {backend}")
    ...
    for loop in range(n_loops):
        for i in range(n_cands):
            tier = _DETERMINISTIC_SCRUB_TIERS[(loop * n_cands + i) % len(_DETERMINISTIC_SCRUB_TIERS)]
            cand = _generate_once(
                backend,
                base_url or "",
                model or "",
            ...
            if cand in seen_candidates:
                # Deterministic generators converge (deep_rewrite is a fixpoint
                # without an active probe), so a repeated candidate adds nothing.
                continue
            ...
            attempts.append(
                (
                    ...
                    {
                        "loop": loop,
                        "tier": tier if backend == "deterministic" else None,
